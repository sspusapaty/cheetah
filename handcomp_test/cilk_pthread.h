...
pthread_mutex_t m; // make this handle specific
void cilk_pthread_exit(void *value_ptr) { // handle return pointer properly
    if (!cilk_is_worker()) {
        ...
        for (unsigned i = 0; i < w->g->options.nproc; ++i) {
            if (i != w->self){
                ...
                s = pthread_cancel(w->g->threads[i]);
